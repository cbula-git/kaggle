...
Usage:
    from nfl_data_loader import NFLDataLoader
...
class NFLDataLoader:
    ...
    def __init__(self, data_dir: str = 'consolidated_data'):
        """
        ...
        if not self.data_dir.exists():
            raise FileNotFoundError(
                ...
                "Please run consolidate_nfl_data.py first."
            )
...
def load_input(data_dir: str = 'consolidated_data') -> pd.DataFrame:
    """Quick load master input data."""
    return NFLDataLoader(data_dir).load_input()


def load_output(data_dir: str = 'consolidated_data') -> pd.DataFrame:
    """Quick load master output data."""
    return NFLDataLoader(data_dir).load_output()


def load_play_level(data_dir: str = 'consolidated_data') -> pd.DataFrame:
    """Quick load play-level data."""
    return NFLDataLoader(data_dir).load_play_level()


def load_player_analysis(data_dir: str = 'consolidated_data') -> pd.DataFrame:
...
if __name__ == "__main__":
    ...
    print(loader.get_dataset_info().to_string(index=False))
    
    print("\nLoading sample data...")
    input_df = loader.load_input()
    print(f"Input data shape: {input_df.shape}")
    print(f"Columns: {', '.join(input_df.columns[:5])}...")
